...
<br>
 ...
 - **Finalizers**
   ...
   - finalizer는 객체가 소멸될 때 직접적으로 메모리를 반납할 수 있도록 호출되기로 약속이 된 메소드이다.<br>
     ex) InputStream 과 OputStream은 각각 close() 함수를 별도로 제공 -> 현재는 쓰지 않는 것을 권장.<br>
 
   - 언제 실행이 될 지 알 수 없으며, 가비지 컬렉션(garbage collection)에 의해서 호출되기 때문에 임의의 시점에 실행이 된다. 
...
<br>
  ...
  * @param, @return, @throws, @deprecated 순으로 사용된다. <br>
